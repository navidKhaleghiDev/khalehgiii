			...
			<Card
				...
				<div className={overflowUiStyle.div}>
					<Typography
						size="body4"
						type="div"
						className={`${overflowUiStyle.typo} ${!isHeader ? 'font-bold' : ''} `}>
						{scannedFile.file_name}
					</Typography>
				</div>
				<div className={overflowUiStyle.div}>
					<Typography size="body4" type="div" className={`${overflowUiStyle.typo} uppercase `}>
				...
				<Typography
					...
					{isHeader ? (
						...
						<span className="flex justify-around items-center border rounded-md whitespace-nowrap overflow-hidden overflow-ellipsis">
							<CircleBg bgColor={scannedFile.yara_scan_result ? 'bg-red-600' : 'bg-green-400'} />
							{scannedFile.yara_scanner_status}
						</span>
					)}
				</Typography>
				<Typography
					...
					{isHeader ? (
						...
						<span className="flex justify-around items-center border rounded-md">
							<CircleBg bgColor={scannedFile.clamav_scan_result ? 'bg-red-600' : 'bg-green-400'} />
							{scannedFile.clamav_scanner_status}
						</span>
					)}
				</Typography>
				<Typography
					...
					{isHeader ? (
						...
						<span className="flex justify-around items-center border rounded-md">
							<CircleBg
								bgColor={scannedFile.antiviruses_scan_result ? 'bg-red-600' : 'bg-green-400'}
							/>
							{scannedFile.antiviruses_scanner_status}
						</span>
					)}
				...
				<div className="px-3 w-2/12 flex justify-center text-center break-words uppercase">
					{!isHeader && onOpenDetailModal ? (
						<IconButton icon={moreIcon} color="neutralNoBg" onClick={onOpenDetailModal} />
					) : (
