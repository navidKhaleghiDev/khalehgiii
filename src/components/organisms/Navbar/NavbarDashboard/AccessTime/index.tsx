...
import { E_USERS_DAAS_UPDATE_USAGE, E_USERS_PROFILE } from '@src/services/users/endpoint';
...
export function AccessTime() {
	...
	const timeLimitValueInHour = data?.data?.daas_configs?.time_limit_value_in_hour ?? 0;

	const usageInMinute = data?.data.usage_in_minute ?? 0;

	return (
		<>
			<Card color="neutral" className="flex items-center justify-between w-64 px-2">
