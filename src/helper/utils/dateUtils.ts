...
const firstDayOfNextWeek = new Date(today.setDate(today.getDate() + 7 - today.getDay() + 1));
const daysUntilNextSaturday = 6 - now.getDay() + 7; // Calculate the number of days until the next Saturday
const nextSaturday = new Date(today.setDate(today.getDate() + daysUntilNextSaturday));

export { tomorrow, firstDayOfNextWeek, lastDayOfWeek, nextSaturday };

export const firstDayOfNextMonth = new Date(today.getFullYear(), today.getMonth() + 1, 1);
...
export const getNextSaturday = (): Date => {
	...
	const daysUntilNextSaturday = currentDayOfWeek <= 6 ? 6 - currentDayOfWeek + 1 : 7; // Calculate the number of days until the next Saturday
